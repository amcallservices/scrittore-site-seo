...
export default async function ResourcesPage({ params }: Props) {
  ...
  return <main dir={copy[locale].direction || "ltr"} lang={locale} className="resources-page">
    ...
    <header className="site-header"><Link href={`/${locale}`} className="brand brand-logo" aria-label="Scrittore Site"><img src="/brand/scrittore-site-logo.png" alt="Scrittore Site" /></Link><nav aria-label={ui.nav}><Link href={`/${locale}`}>{ui.all}</Link><Link href={`/${locale}/come-funziona`}>{copy[locale].nav[1]}</Link><a href={appUrl} target="_blank" rel="noopener noreferrer">{copy[locale].primary} ↗</a></nav></header>
