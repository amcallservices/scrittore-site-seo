...
export default async function ResourceArticle({ params }: Props) {
  ...
  return <main dir={copy[locale].direction || "ltr"} lang={locale} className="article-page">
    ...
    <header className="site-header"><Link href={`/${locale}`} className="brand brand-logo" aria-label="Scrittore Site"><img src="/brand/scrittore-site-logo.png" alt="Scrittore Site" /></Link><nav aria-label={ui.nav}><Link href={`/${locale}`}>Home</Link><Link href={`/${locale}/risorse`}>{ui.nav}</Link><a href={appUrl} target="_blank" rel="noopener noreferrer">{ui.begin} ↗</a></nav></header>
